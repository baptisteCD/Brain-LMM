...
od=${wd}/BLUP_Bslne_bdsz # Where all blup outputs and intermediary files will be stored (as well as job log files)
cd ${od}
...
We now assume you have an indepdenent sample, processed the same way as the discovery sample  
BOD files have been created for this independent sample, which may be another cohort (e.g. HCP) or a later release of the UKB (UKB replication) as per our manuscript  

## BLUP weights on full discovery sample

We have combined the 2 steps above into a single script for simplicity  
...
# In our example the first column is the predicted variable (used in training), the 4th column is the new variable that we are trying to predict. In some cases the names are the same, the differences come from different naming conventions in the ukbtools package between our data update.  
