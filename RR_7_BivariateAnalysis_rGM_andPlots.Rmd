...
# Open data and subset into discovery and replication

```{R, message=FALSE, eval=F}

TT=read.csv("UKB_phenotypes_15K_allData_regVars_Jan2019Update.csv")
# Note, I use read.csv here, which is a bit slow. Though, I found that using read_csv makes a df and does not allow to call TT[,columnName]
...
cd ${od}
${bind}/qsubshcom "echo \${TASK_ID}.\$(sed -n \"\${TASK_ID}{p;q}\" "${wd}"/UKB_phenotypesPairs_bivariate_Bsln_bdsz.txt) |;  "${bind}"/gcta64 --reml-bivar 1 2 --grm-bin "${wd}"/BodFiles/allVerticesBRM_QC --pheno  "${wd}"/Phenotypes15K_Jan2019Update/UKB_phenotypes_bivariate_reg_Bsln_bdsz/\$(sed -n \"\${TASK_ID}{p;q}\" "${wd}"/UKB_phenotypesPairs_bivariate_Bsln_bdsz.txt).txt --out  "${od}"/BivModel.\$(sed -n \"\${TASK_ID}{p;q}\" "${wd}"/UKB_phenotypesPairs_bivariate_Bsln_bdsz.txt).outliers.UKB --reml-maxit 10000 --reml-bivar-lrt-rg 0 --thread-num 5 " 5 20G GREML_all 48:00:00 "-array=1-1378 -acct=UQ-IMB-CNSG" 
