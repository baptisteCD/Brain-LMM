...
Note that we dowloaded the MRI images by batches of 500 (maximum number of download using **ukbfetch**)  
Each batch is numbered and stored in a separate folder called *batchXX*  
The .lis files were created by **ukbfetch** at the end of each dowload  
...
T1w and T2 FLAIR are processed together if both are available  
Otherwise T1w are processed by themselves  
...
SUBJECTS_DIR=\"/gpfs1/scratch/group30days/cnsg_park/uqbcouvy/UKB_MRI_may18/FS/batch"${batch}"/\" |;
...
# If space or number of files is a limitation, you can delete all FreeSurfer files at the end of the processing by adding the extra command: 
...
You may want to re-run processing for those individuals or use this script to check nothing happened to the job  
